...
export default function AdminCategoryManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = isEditing
      ? { id: editId, name, slug, imageUrl, cta }
      : { name, slug, imageUrl, cta };
  ...
  if (fetching) {
    return (
      <div className="h-96 flex items-center justify-center gap-2 text-slate-400 text-sm">
        <Loader2 className="w-5 h-5 animate-spin text-amber-500" /> Connecting
        Category Engine...
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white border border-slate-100 p-5 rounded-2xl shadow-xs gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
            <Layers className="w-5 h-5 text-amber-500" /> Store Category
            Manager
          </h1>
          <p className="text-xs text-slate-400 mt-0.5">
            Organize your store products into clean and browsable categories.
          </p>
        </div>
        <button
          onClick={() => {
      ...
      <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-xs">
        <div className="flex items-center gap-2 border-b border-slate-100 pb-3 mb-4">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400">
              Current category mapping loaded into the database.
            </p>
          </div>
        </div>
        ...
        {categories.length > 0 ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 text-slate-400 text-[10px] font-bold uppercase tracking-wider border-b border-slate-100">
                  ...
                  <th className="py-3 px-4">CTA Text</th>
                    ...
                    <td className="py-3 px-4">
                      <div className="w-10 h-10 rounded-xl overflow-hidden bg-slate-50 border border-slate-200 p-1 flex items-center justify-center">
                        ...
                        <img
                          src={
                            cat.imageUrl ||
                            "https://images.unsplash.com/photo-1523381210434-271e8be1f52b?auto=format&fit=crop&w=80&q=80"
                          }
                          alt="thumb"
          ...
          <div className="text-center py-10 text-xs text-slate-400">
            No categories found. Click "+ Add New Category" button to begin!
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-xl border border-slate-100 max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6 relative space-y-6">
            ...
            <button
              ...
              className="absolute top-4 right-4 p-1.5 hover:bg-slate-100 rounded-xl text-slate-400 hover:text-slate-600 transition-all"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                ...
                {editId ? "Update Category Node" : "Create New Store Category"}
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">
                Fill out parameters below to append or rewrite categories in
                database.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
              ...
              <form onSubmit={handleSubmit} className="md:col-span-3 space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Explore All Sharee"
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-amber-500 text-slate-800"
                  />
                </div>
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        ...
                        {editId ? "Update Category" : "Save Node"}
                      </>
                    )}
                  </button>
                  <button
              ...
              <div className="md:col-span-2 bg-slate-50 p-4 border border-slate-100 rounded-2xl flex flex-col items-center justify-center space-y-3 shadow-inner">
                ...
                <div className="flex flex-col items-center space-y-2 py-4">
                  ...
                  <span className="text-[10px] text-amber-600 text-center max-w-[140px] truncate">
                    {cta || "Explore All..."}
                  </span>
                </div>
              </div>
